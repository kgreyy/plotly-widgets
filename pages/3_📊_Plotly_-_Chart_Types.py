...
def fetching_plotly_figures():
    ...
    d[2] = go.Figure(
        data=[
            go.Scatter(x=random_x, y=y, mode=t, name=t)
            for (y, t) in zip(random_y, trace_info)
        ]
    )
    d[3] = go.Figure(
        data=go.Scatter(
    ...
    fig_4 = go.Figure()
    fig_4.add_trace(
        go.Scatter(x=t, y=np.sin(t), name="sin", marker_color="rgba(152, 0, 0, .8)")
    )
    fig_4.add_trace(
        go.Scatter(x=t, y=np.cos(t), name="cos", marker_color="rgba(255, 182, 193, .9)")
    )
    fig_4.update_traces(mode="markers", marker_line_width=2, marker_size=10)
    fig_4.update_layout(
        title="Styled Scatter", yaxis_zeroline=True, xaxis_zeroline=False
    )
    d[4] = fig_4
    data = pd.read_csv(
    ...
    df = px.data.tips()
    fig_7 = px.scatter(
        df, x="total_bill", y="tip", facet_col="sex", width=800, height=400
    )
    fig_7.update_layout(
    ...
    fig_10.add_trace(
        go.Scatter(
            ...
            marker=dict(
                color="rgba(204, 204, 204, 0.95)", line_color="rgba(217, 217, 217, 1.0)"
            ),
        )
    )
    fig_10.update_traces(
        mode="markers", marker=dict(line_width=1, symbol="circle", size=16)
    )
    ...
    d[12] = go.Figure(
        data=[
            go.Scatter(
                x=month, y=y, name=name, line=dict(color=color, width=4, dash=dash)
            )
    ...
    fig_14 = go.Figure(
        data=[
            go.Scatter(x=x, y=y, name="linear", line_shape="linear"),
    ...
    d[16] = px.bar(
        data_canada[data_canada.country == "Canada"],
        x="year",
        y="pop",
        hover_data=["lifeExp"],
    )
    ...
    df = px.data.tips()
    d[24] = px.box(
        df, x="time", y="total_bill", color="smoker", points="all", notched=True
    )
    df = px.data.tips()
    ...
    fig_27.update_layout(
        violingap=0, violinmode="overlay", height=300, width=600, margin_t=20
    )
    d[27] = fig_27
    data = (
    ...
    fig_30 = go.Figure(
        data=[
            go.Histogram(x=x0, marker_color="#EB89B5", name="Pink"),
    ...
    df = px.data.tips()
    fig_31 = px.histogram(
        df, x="total_bill", y="tip", color="sex", marginal="box", hover_data=df.columns
    )
    ...
    d[32] = ff.create_distplot(hist_data, group_labels, bin_size=0.2)
    d[33] = go.Figure(
        data=go.Heatmap(
    ...
    d["35b"] = px.imshow(
        np.corrcoef(data), color_continuous_scale="RdBu_r", origin="lower"
    )
    df = px.data.tips()
    d[36] = px.density_heatmap(df, x="total_bill", y="tip", histfunc="avg")
    d[37] = px.density_heatmap(
    ...
    df = px.data.tips()
    d[41] = px.scatter(
        df, x="total_bill", y="tip", color="sex", facet_col="day", marginal_x="box"
    )
    # deleted figure 38
    df = pd.read_csv(
    ...
    d[43] = go.Figure(
        data=go.Splom(
            ...
            text=df["class"],
            marker=dict(
                color=index_vals, showscale=False, line_color="white", line_width=0.5
            ),
        )
    )
    df = pd.read_csv(
...
for i in range(2):
    with cols_scatter_1[i]:
        st.image(f"images/charts-scatter-{i}.png")
for i in range(2):
    with cols_scatter_2[i]:
        st.image(f"images/charts-scatter-{i+2}.png")

st.header("Line charts")
with st.expander("Code & interactive graphs"):
    for code, fig in zip(
        [code_11, code_12, code_13, code_14, code_15], [d[i] for i in range(11, 16)]
    ):
        st.code(code, language="python")
        st.plotly_chart(fig)
...
for i in range(2):
    with cols_line[i]:
        st.image(f"images/charts-line-{i}.png")

st.header("Bar charts")
with st.expander("Code & interactive graphs"):
    for code, fig in zip(
        [code_16, code_17, long_df, code_18, code_19, code_20, code_21],
        [d[16], d[17], None, d[18], d[19], d[20], d[21]],
    ):
        if fig is not None:
            st.code(code, language="python")
            st.plotly_chart(fig)
...
for i in range(2):
    with cols_bar_1[i]:
        st.image(f"images/charts-bar-{i}.png")
for i in range(2):
    with cols_bar_2[i]:
        st.image(f"images/charts-bar-{i+2}.png")
...
for i in range(2):
    with cols_box[i]:
        st.image(f"images/charts-box-{i}.png")

st.header("Violin plots")
with st.expander("Code & interactive graphs"):
    for code, fig in zip(
        [code_25, code_26, code_27, code_28], [d[25], d[26], d[27], d[28]]
    ):
        st.code(code, language="python")
        st.plotly_chart(fig)
...
for i in range(2):
    with cols_violin[i]:
        st.image(f"images/charts-violin-{i}.png")

st.header("Histograms")
with st.expander("Code & interactive graphs"):
    for code, fig in zip(
        [code_29, code_30, code_31, code_32], [d[29], d[30], d[31], d[32]]
    ):
        st.code(code, language="python")
        st.plotly_chart(fig)
cols_histogram = st.columns(2)
for i in range(2):
    with cols_histogram[i]:
        st.image(f"images/charts-histogram-{i}.png")

st.header("Heatmaps")
with st.expander("Code & interactive graphs"):
    for code, fig in zip(
        [code_33, code_34, code_35, code_35b, code_36, code_37],
        [d[33], d[34], d[35], d["35b"], d[36], d[37]],
    ):
        st.code(code, language="python")
        st.plotly_chart(fig)
...
for i in range(2):
    with cols_heatmap_1[i]:
        st.image(f"images/charts-heatmap-{i}.png")
for i in range(2):
    with cols_heatmap_2[i]:
        st.image(f"images/charts-heatmap-{i+2}.png")
...
for i in range(2):
    with cols_marginal[i]:
        st.image(f"images/charts-marginal-{i}.png")
...
for i in range(2):
    with cols_scatterplot[i]:
        st.image(f"images/charts-scatterplot-{i}.png")
...
for i in range(2):
    with cols_table[i]:
        st.image(f"images/charts-table-{i}.png")
